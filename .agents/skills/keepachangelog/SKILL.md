---
name: keepachangelog
description: How to write and maintain a real CHANGELOG.md following the Keep a Changelog 1.1.0 format (https://keepachangelog.com/en/1.1.0/) - project-agnostic. Covers the six real change categories, the Unreleased section, version/date headers, comparison links, and yanked releases. Use when creating a new CHANGELOG.md, adding an entry for a release, or reviewing one for format compliance.
---

# Keep a Changelog

A changelog is for the humans reading your release, not a dump of
`git log`. This is the real spec at
...
- **Every version shows its real release date** — `YYYY-MM-DD`, no
...
### Added
- Real thing that landed on `main` but hasn't shipped in a release yet.

## [1.4.2] - 2026-03-14

### Fixed
- The real bug, described from the user's point of view, not the diff's.
...
**`[Unreleased]` stays at the top, always** — it's where real,
already-merged changes go the moment they land, before there's a
version number for them yet. At release time, rename it to the real
new version + date and open a fresh empty `[Unreleased]` above it —
don't leave the previous release's real changes mixed in with the next
one's.
...
real diff between that version's tag and the one before it (or `HEAD`
for `Unreleased`) — not required by the spec, but the real, practical
way to make "versions are linkable" actually useful on GitHub/GitLab.

## Yanked releases

A version pulled after release (a real, broken build; a security
issue) still gets a real entry — don't delete it, that erases the
...
- Don't write `## [1.4.2] - 2026-03-14` and then just paste raw commit
  subjects underneath — a changelog entry describes the real, observable
...
- Don't skip a version's section because "it was just a dependency
  bump" — if it shipped, it gets a real entry (even a one-line
  `Changed` note), or the "one entry per version" principle is broken.
- Don't backdate or guess a release date — use the real date the
