...
description: How to write real Conventional Commits (v1.0.0 spec, https://www.conventionalcommits.org/en/v1.0.0/) - project-agnostic. Covers the type(scope) message grammar, the real type vocabulary, how BREAKING CHANGE is expressed, and how this maps directly to SemVer bumps. Use when writing a commit message, setting up commit-lint tooling, or reviewing commit history for a release.
---
...
persisted so a session survives past the access token's real expiry
without forcing a re-login.

Refs: #482
```

## The real, load-bearing types

Only two types carry real, spec-mandated meaning — everything else is
convention, not requirement:

| Type | Real meaning | SemVer bump |
|---|---|---|
| `fix` | A real bug patch | `PATCH` |
| `feat` | A real new feature | `MINOR` |
...
**Other real, widely-used types** (not spec-mandated, but standard
...
Optional, in parentheses right after the type — names the real part of
the codebase affected: `feat(parser): ...`, `fix(cli): ...`. Keep it to
a real, existing subsystem name, not a made-up one per commit.

## Breaking changes — two real ways to say it
...
or
```
feat: add real streaming response support
...
case-insensitive. State the real, concrete thing that breaks (a
...
line — the same real shape git already uses for `Signed-off-by:`:
```
Reviewed-by: real-reviewer
Refs: #482
```

## Real rules worth remembering

- The description must immediately follow `type(scope):` on the same
  line, and should read as a real, present-tense instruction ("add X",
  not "added X" or "adds X") — matches how `git log --oneline` reads
  best.
- A body, if present, starts one real blank line after the description
  — free-form prose explaining *why*, not restating *what* the diff
  already shows.
- This only constrains the **commit message**, not commit granularity —
  one real logical change per commit is still a separate, good practice
  this spec doesn't itself enforce.

## Why this matters beyond readability

`feat`/`fix`/`!` map directly onto [SemVer](../semver/SKILL.md)'s
MAJOR/MINOR/PATCH — a real changelog-generation or release-automation
tool can compute the next real version number **from commit history
alone**, and a [Keep a Changelog](../keepachangelog/SKILL.md) entry can
be drafted from the same real commit messages, if they're written this
