...
This is a promise to your real users, not a changelog summary — the
...
- **MAJOR** for any backward-incompatible change — a removed function,
  a changed required parameter, a changed default that alters real
  behavior.
...
- **`1.0.0` is the real commitment point** — ship it once there's a
  real, stable public API real users depend on. Per the spec's own FAQ:
...
- A pre-release suffix (`-alpha`, `-rc.1`, ...) comes right after
  `PATCH`, and marks the version as **not yet stable** — real tooling
  should not install it over a real caller's explicit opt-in.
- Build metadata (`+001`, `+build.5`) can follow the version *or* the
  pre-release tag, and is **ignored for precedence** — `1.0.0+001` and
  `1.0.0+002` are the exact same real version as far as comparison goes,
  the suffix is just a real, informational tag (a commit SHA, a CI build
  number).

## Real precedence rules (how two versions actually compare)
...
Real worked example, low to high:
```
1.0.0-alpha < 1.0.0-alpha.1 < 1.0.0-alpha.beta < 1.0.0-beta
  < 1.0.0-beta.2 < 1.0.0-beta.11 < 1.0.0-rc.1 < 1.0.0
```

## Real validation regex

```regex
^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$
```
Use this (or your language's real equivalent with named capture groups,
...
real pre-1.0 release (`0.2.0`, `0.3.0`, ...) — don't invent your own
...
- Don't treat build metadata as meaningful for version *ordering* — if
  ...
  real semver-aware comparison library that already does.
