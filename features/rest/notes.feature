Feature: BDD Framework for the rest-api test fixture's Notes CRUD
  ...
  Scenario: Creating, reading, updating, and deleting a Note
    ...
    And HelmRelease known as "<NotesRelease>":
      | PROPERTY  | VALUE                |
      | chart     | <RestApiHelmChart>   |
      | name      | sandbox-notes-release |
      | namespace | thomas-helm-test   |
    When I upgrade HelmRelease known as "<NotesRelease>" with:
      | OPTION             | VALUE |
      | --install          | True  |
      | --atomic            | True  |
  ...
  Scenario: Rejecting a request for a Note that was never created
    ...
    And Helm Chart known as "<RestApiHelmChart>":
      | PROPERTY | VALUE                 |
      | chart    | <RestApiChartDirectory> |
    And HelmRelease known as "<NotesMissingRelease>":
      | PROPERTY  | VALUE                        |
      | chart     | <RestApiHelmChart>           |
      | name      | sandbox-notes-missing-release |
      | namespace | thomas-helm-test           |
    When I upgrade HelmRelease known as "<NotesMissingRelease>" with:
      | OPTION             | VALUE |
      | --install          | True  |
      | --atomic            | True  |
      | --create-namespace  | True  |
