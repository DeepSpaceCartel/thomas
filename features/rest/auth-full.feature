Feature: BDD Framework for the rest-api fixture's five auth methods (full syntax)
  Scenario: API key, Basic, Bearer, OAuth2, and OpenID Connect authentication
    ...
    When I upgrade Helm Release known as "<AuthRelease>" with:
      | OPTION            | VALUE |
      | --install         | True  |
      | --atomic          | True  |
    ...
    Given Service known as "<AuthService>":
      ...
      | app.kubernetes.io/instance | thomas-auth-release |
    And HTTP Endpoint "<AuthApi>" on "<AuthService>" port "8000"

    When I send a POST request to Endpoint known as "<AuthApi>" path "/users/" with:
      | TYPE  | KEY             | VALUE       |
    ...
    When I send a GET request to Endpoint known as "<AuthApi>" path "/auth/bearer/whoami" with:
      | TYPE   | KEY           | VALUE                  |
    ...
    When I send a POST request to Endpoint known as "<AuthApi>" path "/oauth/token" with:
      | TYPE   | KEY          | VALUE                                                                        |
      | HEADER | Content-Type | application/x-www-form-urlencoded                                           |
      | BODY   |              | grant_type=authorization_code&code=<AuthCode>&client_id=thomas-client&redirect_uri=https://client.example/callback |
    Then the response status is 200
    ...
    When I send a GET request to Endpoint known as "<AuthApi>" path "/auth/oauth2/whoami" with:
      | TYPE   | KEY           | VALUE                    |
      | HEADER | Authorization | Bearer <OAuthAccessToken> |
    Then the response status is 200

    When I send a GET request to Endpoint known as "<AuthApi>" path "/oauth/userinfo" with:
      | TYPE   | KEY           | VALUE                    |
      | HEADER | Authorization | Bearer <OAuthAccessToken> |
    Then the response status is 200
    Then the command result data has:
    ...
    When I send a GET request to Endpoint known as "<AuthApi>" path "/auth/openid/whoami" with:
      | TYPE   | KEY           | VALUE             |
