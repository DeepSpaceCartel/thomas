...
features/
  ...
  step_definitions/
    ...
    helm-release.step.ts HelmRelease-scoped: install, upgrade, uninstall, rollback,
  ...
  support/
    ...
    helm/          HelmChart, HelmRepo, HelmRelease, chart_ref_args, and real chart-fetching logic
...
their alias fields to a **string** (a path, a URL, a ref). `HelmRelease` is
...
`HelmRelease` needs the object itself to build the right command later. It
uses a separate `(alias) => HelmChart | undefined` resolver
(`helmReleaseFromTable` in `support/helm/helm_release.ts`), not the shared
...
`RestEndpoint` is a second case of the same `HelmRelease.chart` exception:
...
preceding `HelmRelease` install used `--atomic`, which blocks until the
...
- Every scenario that mutates real local `helm` state (repo add/remove) or
  real cluster state (`HelmRelease` install/uninstall) does so idempotently
...
- `HelmRelease` scenarios deploy into a dedicated `thomas-helm-test`
