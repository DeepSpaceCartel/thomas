...
and verified
- see `Thomas/AGENTS.md`'s Status section and `.agents/skills/
...
4. `npx tsc --noEmit` clean; `npx cucumber-js --dry-run --format
   snippets` clean.
5. `npm test` twice in a row - `kubectl get all -n thomas-helm-test` /
   `helm list` both empty afterward, both runs.
6. Update `Thomas/README.md` (any new divergence - most likely the
